...
class Hampir_m extends CI_Model {
    ...
    private function _get_datatables_query() {
        ...
        $this->db->or_where('tb_status.status_name', $statusnow);
        $this->db->where('tb_rekom.deadline <=',date('Y-m-d', strtotime($date . ' + 7 days')) );
        $this->db->where('tb_rekom.deadline >=' , $date);

        $this->db->or_where('tb_status.status_name', $statusnow2);
        $this->db->where('tb_rekom.deadline <=',date('Y-m-d', strtotime($date . ' + 7 days')) );
        $this->db->where('tb_rekom.deadline >=' , $date);
        
        $i = 0;
        foreach ($this->column_search as $hampir) { // loop column 
            if(@$_POST['search']['value']) { // if datatable send POST for search
                if($i===0) { // first loop
                    $this->db->group_start(); // open bracket. query Where with OR clause better with bracket. because maybe can combine with other WHERE with AND.
                    $this->db->like($hampir, $_POST['search']['value']);
                } else {
                    $this->db->or_like($hampir, $_POST['search']['value']);
                }
